...
# - JAX backend specific function do not add to loader
PRNG = jax.random.PRNGKey(int(time()))
...
def zeros(shape, dtype=floatx()):
    ...
    """
    return jxnp.zeros(shape, dtype=dtype)
...
def ones(shape, dtype=floatx()):
    ...
    """
    return jxnp.ones(shape, dtype=dtype)
...
def fill(shape, fill_value):
    ...
    """
    return jxnp.full(shape, fill_value)


def normal(shape, mean='not_used', stddev='not_used'):
    ...
    """
    return jax.random.normal(PRNG, shape=shape)
...
def range(start, stop=None, step=1, dtype=intx()):
    ...
    """
    return jxnp.arange(start, stop=stop, step=step, dtype=dtype)
...
def prod(tensor, axis=None, keepdims=False):
    ...
    """
    return jxnp.prod(tensor, axis=axis, keepdims=keepdims)
...
def max(tensor, axis=None, keepdims=False):
    ...
    Returns:
        ndarray: The maximum of ``tensor``, along the axis if specified.
    """
    return jxnp.amax(tensor, axis=axis, keepdims=keepdims)
...
def min(tensor, axis=None, keepdims=False):
    ...
    return jxnp.amin(tensor, axis=axis, keepdims=keepdims)
...
def sum(tensor, axis=None, keepdims=False):
    ...
    """
    return jxnp.sum(tensor, axis=axis, keepdims=keepdims)
...
def mean(tensor, axis=None, keepdims=False):
    ...
    """
    return jxnp.mean(tensor, axis=axis, keepdims=keepdims)
...
def roll(tensor, shift, axis):
    ...
    Returns:
        [type]: [description]
    """
    return jxnp.roll(tensor, shift, axis)
...
def concatenate(tup, axis=0):
    ...
    """
    return jxnp.concatenate(tup, axis=axis)
...
def as_numpy_array(tensor):
    ...
    """
    return jxnp.asarray(tensor)


def reshape(tensor, shape):
    "reshape tensor"
    return jxnp.reshape(tensor, shape)


def is_tensor(a):
    "check if the given object is a tensor"
    return isinstance(a, jxnp.ndarray)
...
def tensor_equal(tensor1, tensor2):
    ...
    """
    return jxnp.array_equal(tensor1, tensor2)
...
def add(tensor1, tensor2):
    ...
    """
    return jxnp.add(tensor1, tensor2)
...
def subtract(tensor1, tensor2):
    ...
    Args:
        ...
        tensor2 (ndarray): right tensor.
    """
    return jxnp.subtract(tensor1, tensor2)
...
def multiply(tensor1, tensor2):
    ...
    Args:
        ...
        tensor2 (ndarray): right tensor.
    """
    return jxnp.multiply(tensor1, tensor2)
...
def divide(numerator, denominator):
    ...
    """
    return jxnp.divide(numerator, denominator)
...
def mod(numerator, denominator):
    ...
    Args:
        ...
        tensor2 (ndarray): denominator tensor.
    """
    return jxnp.mod(numerator, denominator)
...
def clip(tensor, min_val=0, max_val=None):
    ...
    """
    return jxnp.clip(tensor, a_min=min_val, a_max=max_val)


def abs(tensor):
    "Calculate the absolute value element-wise."
    return jxnp.absolute(tensor)


def broadcasted_norm(tensor):
    "Norm broadcasted accross dimensions"
    return jxnp.sum(jxnp.abs(tensor)**2, axis=-1)**0.5
...
def norm(tensor, ord=None, axis=None, keepdims=False):
    ...
    """
    return jxnp.linalg.norm(tensor, ord=ord, axis=axis, keepdims=keepdims)
...
def randint(low, high=None, shape=None, dtype=intx):
    ...
    """
    return jax.random.randint(PRNG,
                              minval=low,
                              maxval=high,
                              shape=shape,
                              dtype=dtype)
...
def shuffle(tensor, axis=0):
    ...
    """
    return jax.random.shuffle(PRNG, tensor, axis=axis)
...
def full_shuffle(tensor):
    ...
    for idx in range(len(tensor.shape)):
        tensor = shuffle(tensor, axis=idx)
    return tensor
...
def take(tensor, indices, axis=None):
    ...
    """
    return jxnp.take(tensor, indices, axis=axis)
...
def top_k_indices(tensor, k):
    ...
    idxs = jxnp.argpartition(tensor, k)[k:]

    # ! mind the - argsort return in increasing order we want largest
    return idxs[jxnp.argsort(-tensor[idxs])]
...
def unique_with_counts(tensor):
    ...
    """
    return jxnp.unique(tensor, return_index=True, return_counts=True)
...
def bincount(tensor, weights=None, minlength=0):
    ...
    """
    return jxnp.bincount(tensor, weights=weights, minlength=minlength)
